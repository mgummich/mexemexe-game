...
| TM-15 | **No security headers on the static host** — no CSP, no `X-Content-Type-Options`, no `Referrer-Policy`, no frame policy | deployment | P2 | **open** | `nginx.conf` serves the shell with nginx defaults only. A canvas game with no third-party scripts is a small XSS target, but CSP is cheap and `nosniff`/`Referrer-Policy` are one line each → Phase 80 |
| TM-16 | **The server container runs as root** | deployment | P2 | **open** | neither image declares `USER`; a process escape starts as uid 0. The server needs no privileged port (8787) and writes nothing to disk → Phase 80 |
