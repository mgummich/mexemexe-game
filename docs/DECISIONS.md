...
| D16 | Screenshots are captured and read by humans; there is no baseline image diff. | [TESTING.md](TESTING.md) | Pixel diffing: renderer-dependent bytes would fail on a different machine, which teaches everyone to ignore the check |
