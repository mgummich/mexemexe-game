...
| INV-A2 | The AI reads only the active player's hand and public state — never an opponent's cards, the pile's order or the deal seed. Structural, not conventional: an engine is handed an `AiObservation` from `observeForAi`, the hidden halves are absent from that object, and only that function can build one, so passing authoritative state does not type-check. | type + test — `tests/ai.test.ts` observation cases (redaction, own hand kept, input untouched) and the hidden-information case: two states differing only in the opponent's hand yield the same decision | SCN-17 |
| INV-A3 | An AI failure or timeout cannot corrupt committed state: the fallback is `drawAndEndTurn`, which is a legal move. A thrown engine, an exhausted budget and a confirm the rules refuse all end the turn that way rather than leaving the seat holding it. | test — `tests/ai.test.ts` fallback + budget cases, `tests/match.test.ts` refused-confirm fallback | SCN-15, SCN-16 |
...
| INV-A5 | An AI decision is a pure function of the state and the tier. The search spends a deterministic trial budget and reads no clock, so the same state always yields the same move — `decideSliced` included. | test — `tests/ai.test.ts` decision reproducibility (repeated runs of the densest legal table); `tests/boundaries.test.ts` keeps `src/ai` clock-free; `tests/property/ai.property.test.ts` over generated states | SCN-13, SCN-15 |
| INV-A6 | Deciding never writes to the state it decides about: the observation is a frozen copy, and the draft an engine builds lives in a `DraftEditor` until `LocalMatch` dispatches it. | test — `tests/ai.test.ts` no-mutation cases, `tests/property/ai.property.test.ts` over generated states | SCN-14, SCN-16 |
| INV-A7 | Explainability is observational. An engine records what its search did (`DecisionTrace`) and `createAi` attaches the reason class on the way out; nothing in a decision, search or rule reads either back. The trace describes the AI's *own* work (what it rearranged, how many candidates it weighed, the chosen draft's features), which debug and balance tooling may read freely; it is derived from the seat's `AiObservation`, so it can no more name an opponent's card or the pile's order than the decision itself could (INV-A2). Only `reason.key` is ever shown to a player. | test — `tests/ai.test.ts` explainability cases (engines return no reason of their own; the attached reason is a pure function of the decision) | SCN-14 |

### Multiplayer — owner: `server/rooms.ts` (`RoomManager`) + `src/net/protocol.ts`

| ID | Invariant | Enforced by | Scenarios |
|---|---|---|---|
| INV-N1 | The server owns authoritative room and match state. Clients submit intents and render what they are given; no client advances `rev` or decides legality. | structure + test — `tests/server/*`, `verify:multiplayer` state-hash agreement | SCN-20, SCN-21, SCN-40 |
| INV-N2 | Opponent hand identities are never transmitted. `buildView` sends own hand + opponent counts only. | test — OH-26, `tests/server/index.integration.test.ts` (privacy in a real frame), `e2e-multiplayer` hand-privacy probe | SCN-23, SCN-36 |
...
| INV-N4 | Seat ownership is unambiguous: one seat per token, one token per seat, for the room's lifetime. | test — `tests/server/rooms.test.ts`, `lobby-soak` (a token never resolves to another seat) | SCN-19, SCN-22, SCN-24, SCN-36, SCN-37 |
...
| INV-N6 | A rematch starts from clean per-match state: state, `rev`, `matchId`, `matchSeats`, timer fields, ready bits and the Mexe-bonus guard are all reset in `recycleForRematch`. | test — `tests/server/rooms.test.ts`, OS-01..OS-16/OS-35 | SCN-25, SCN-38 |
| INV-N7 | The turn timer is server-owned. Clients render `turnMsLeft` and never decide expiry; the running clock is excluded from the state digest. | test — `tests/server/timer.test.ts` (fake clock), `tests/net/room-settings.test.ts` | SCN-26, SCN-39 |
...
| SCN-36 | Room discovery. A room is private at creation, the host lists it, another player finds it in the browser and joins from the card or the invite link → only the bounded listing projection ever crosses the wire, never a room snapshot or a room the caller has not joined. | N2, N4 | `tests/server/discovery.test.ts` (OD-\*), `e2e-multiplayer` OD-03/OD-07/OD-13/OD-30 |
| SCN-37 | Casual matchmaking. Players queue for 2/3/4/any → oldest-first groups form, each player is assigned exactly one seat, a repeated join is answered with the entry it already has and a cancel that lost to a committed match is told `matched`. | N4, S5 | `tests/server/matchmaking.test.ts`, `tests/server/queue.integration.test.ts` (OM-\*), `e2e-multiplayer` OM-07..OM-17 |
| SCN-38 | Party session across matches. Wins, rematch votes, bounded history and the activity feed survive a recycle; reactions are a closed id set, per-seat cooldown, relayed by the server's own value. | N6, S2 | `tests/server/party.test.ts`, `tests/server/party.integration.test.ts` (OS-\*), `e2e-multiplayer` party specs |
| SCN-39 | Absent seat, to the end. A seat gone past the room's grace has its turns played for it, and the `missedTurnLimit`-th consecutive miss ends the match and closes the room, notifying every attached socket. | N7, S4, L3 | `tests/server/timer.test.ts`, `tests/server/rooms.test.ts` sweep, `tests/online-session.test.ts` timeout policy |
| SCN-40 | Budgets under load. Room creation, `list_rooms`, `resync` and matchmaking requests are each refused (or, for `resync`, dropped) at their own budget, and a reaction past its per-seat cooldown is dropped silently — in every case the room model and every other client stay untouched. | N1, D2 | `tests/server/hardening.test.ts` (OH-\*), `tests/server/discovery.test.ts` list budget, `tests/server/queue.integration.test.ts` budget |
| SCN-41 | Adverse delivery. One client's incoming frames are delayed, duplicated, re-delivered stale or dropped on a real socket → the board converges on the server's revision, a duplicate changes nothing, a stale frame is ignored, and a withheld answer to the client's own move is recovered by the pending timeout. | N1, N3, L3 | `e2e-multiplayer/multiplayer.spec.ts` CH-01..CH-05 |
| SCN-42 | Repeated lifecycle. The same room is created, played out, rematched, has a seat dropped and reclaimed mid-match, and is left — over and over on one server process → every cycle ends with no rooms and no connections held, the lifecycle counters match the cycles exactly, and the heap does not climb with the repeats. | N1, N4, L3 | `tests/server/session-soak.integration.test.ts` (nightly at `MEXE_SOAK=1`), `tests/server/lobby-soak.test.ts` |
| SCN-43 | Background and back. An online client is hidden and shown again (the iOS suspend/resume the OS performs on its own) → the session resumes on the socket it already holds, the seat, host and ready bit are what they were, and no second socket or second membership appears. | N4, L3 | `e2e-multiplayer/ios-lobby.spec.ts` LB-48 (WebKit, iPhone viewport), `tests/lifecycle.test.ts`, `tests/net/reconnect.test.ts` OR-22 |
