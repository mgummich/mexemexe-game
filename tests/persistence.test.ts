...
describe('loadSave', () => {
  ...
  it('migrates the old unversioned settings key and removes it', () => {
    ...
    expect(result).toEqual({ version: 1, settings: { ...oldSettings, musicEnabled: false, musicContextAware: true, largeText: false, helperMode: 'standard', batterySaver: false, aiDifficulty: 'smart', aiSpeed: 'normal', aiExplain: 'simple', timerTickSound: true, haptics: false, blitz: false, blitzPanic: true, blitzLastBreath: true }, progress: DEFAULT_PROGRESS, cosmetics: DEFAULT_COSMETICS });
